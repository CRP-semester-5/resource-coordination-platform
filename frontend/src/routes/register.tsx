  ...
  async function handleStep1New(e: React.FormEvent) {
    ...
    try {
      ...
      if (e.response?.status === 409) {
        setError(
          "This email is already registered. Switch to 'Already have an account' to sign in.",
        );
      } else {
    ...
    return (
      ...
      >
        <div className="w-full max-w-md text-center">
          ...
          <p className="text-sm mb-8" style={{ color: "var(--muted-foreground)" }}>
            Click the link in the email to activate your account, then come back here and sign in to
            ...
            <>
              ...
              {mode === "new" && (
                <form onSubmit={handleStep1New} className="space-y-4">
                  ...
                  <button
                    id="register-new-submit"
                    type="submit"
                    disabled={submitting}
                    className="w-full py-2.5 rounded-lg font-semibold text-sm hover:opacity-90 disabled:opacity-60 mt-2"
                    style={{
