...
export const orgsAPI = {
  ...
  getMembers: (orgId: string) => http.get(`/api/v1/organizations/${orgId}/members`),
...
export const requestsAPI = {
  ...
  unapprove: (id: string, orgId?: string) =>
    http.patch(
      `/api/v1/requests/${id}/unapprove`,
      {},
      orgId ? { headers: { "x-organization-id": orgId } } : {},
    ),
  reject: (id: string, rejection_reason: string, orgId?: string) =>
    http.patch(
...
export const donationsAPI = {
  getAll: (orgId: string) =>
    http.get("/api/v1/donations", { headers: { "x-organization-id": orgId } }),
  approve: (id: string) => http.patch(`/api/v1/donations/${id}/approve`),
  reject: (id: string, reason?: string) =>
    http.patch(`/api/v1/donations/${id}/reject`, { rejection_reason: reason }),
};
...
export const inventoryAPI = {
  getAll: (orgId?: string) =>
    http.get("/api/v1/inventory", orgId ? { headers: { "x-organization-id": orgId } } : {}),
  getTransactions: (orgId?: string) =>
    http.get(
      "/api/v1/inventory/transactions",
      orgId ? { headers: { "x-organization-id": orgId } } : {},
    ),
  checkStock: (category: string, quantity: number, orgId?: string) =>
    http.post(
      "/api/v1/inventory/check-stock",
      { category, quantity },
      orgId ? { headers: { "x-organization-id": orgId } } : {},
    ),
  add: (category_id: string, quantity: number, orgId?: string) =>
    http.post(
      "/api/v1/inventory",
  ...
  restock: (id: string, quantity: number) =>
    http.post(`/api/v1/inventory/${id}/restock`, { quantity }),
  allocate: (id: string, quantity: number, code: string) =>
    http.post(`/api/v1/inventory/${id}/allocate`, { quantity, request_code: code }),
  deduct: (
    data: {
      category_id?: string;
...
export const tasksAPI = {
  ...
  assign: (id: string, volunteer_id: string) =>
    http.post(`/api/v1/tasks/${id}/assign`, { volunteer_id }),
  verifyWarehousePickup: (id: string, pin: string) =>
    http.post(`/api/v1/tasks/${id}/verify-warehouse-pickup`, { pin }),
  verifyHandover: (id: string, pin: string) =>
    http.post(`/api/v1/tasks/${id}/verify-handover`, { pin }),
